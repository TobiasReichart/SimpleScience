...
# Gerade Verbindung (Möglichkeit I)
x_line = x1 + (x2 - x1) * t
y_line = y1 + (y2 - y1) * t

# Glatte, monotone Kurve (Möglichkeit II)
x_orange = x1 + (x2 - x1) * t
y_orange = y1 + (y2 - y1) * (1 - (1 - t)**2)

# Wellenförmige Verbindung (Möglichkeit III)
x_green = x1 + (x2 - x1) * t
y_green = y1 + (y2 - y1) * t + 0.35 * np.sin(2 * np.pi * 2 * t) * np.sin(np.pi * t)
...
setup_qualitative_axes(ax, xlim=(-0.2, 6.3), ylim=(-0.2, 3.6), xlabel="x", ylabel="y")
...
# Drei Möglichkeiten
ax.plot(x_line, y_line, lw=2, label="Möglichkeit I", color=ps.colors["purple"])
ax.plot(x_orange, y_orange, lw=2.5, label="Möglichkeit II", color=ps.colors["green"])
ax.plot(x_green, y_green, lw=2, label="Möglichkeit III", color=ps.colors["orange"])

# "Legende" als freie Beschriftung rechts
ax.text(5.65, 2.15, "Möglichkeit I",   fontsize=16, color=ps.colors["purple"])
ax.text(5.65, 1.90, "Möglichkeit II",  fontsize=16, color=ps.colors["green"])
ax.text(5.65, 1.65, "Möglichkeit III", fontsize=16, color=ps.colors["orange"])
