...
# Glatte, monotone Kurve y^
x_green = x1 + (x2 - x1) * t
y_green = y1 + (y2 - y1) * (1 - (1 - t)**2) + 0.1*np.sin(5*t*np.pi)
...
# zwei Möglichkeiten
ax.plot(x_line, y_line, lw=2, label=r"$y(x)$", color=ps.colors["purple"])
ax.plot(x_green, y_green, lw=2.5, label=r"$\hat{y}(x)$", color=ps.colors["green"])

# "Legende" als freie Beschriftung rechts
ax.text(3, 1.8, r"$y(x)$",   fontsize=16, color=ps.colors["purple"])
ax.text(2.9, 2.75, r"$\hat{y}(x)$",  fontsize=16, color=ps.colors["green"])
