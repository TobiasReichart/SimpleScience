...
# Parameterdarstellung der Kurve zwischen A und B
t = np.linspace(0, 1, 400)
x = x1 + (x2 - x1) * t

# Fallende qualitative Kurve:
# steiler Start, flacher werdend gegen B
y = y2 + (y1 - y2) * (1 - t)**2
...
setup_qualitative_axes(ax, xlim=(-0.2, 6.3), ylim=(-0.2, 6.3), xlabel="x", ylabel="y")
...
ax.text(x1, -0.12, r"$x_1$", ha="center", va="top",    fontsize=14, color=ps.colors["gray"])
ax.text(x2, -0.12, r"$x_2$", ha="center", va="top",    fontsize=14, color=ps.colors["gray"])
ax.text(-0.12, y1, r"$y_1$", ha="right",  va="center", fontsize=14, color=ps.colors["gray"])
ax.text(-0.12, y2, r"$y_2$", ha="right",  va="center", fontsize=14, color=ps.colors["gray"])
...
# Kurve (mögliche Rutschbahn)
ax.plot(x, y, "--", lw=2, color=ps.colors["purple"])
ax.text(3.0, 2.55, "mögliche Rutschbahn", fontsize=16, color=ps.colors["purple"])
