...
$$\mathrm{d}s = \sqrt{1 + y^{\prime}(x)^{2}} \mathrm{d}x$$
...
## Bonusaufgabe: Kürzester Weg auf der Kugel

wir modellieren die Erda als perfekte Kugel mit Radius $R$. Ein Punkt auf der Erdoberfläche wird durch einen Ortsvektor $\vec{r} \in \mathbb{R}^{3}$ mit

$$\Vert \vec{r} \Vert = R$$

beschrieben.

Zwei Städte seien durch normierte Richtungsvektoren $\vec{a}, \, \vec{b} \in \mathbb{R}^{3}$ gegeben $\left( \Vert \vec{a} \Vert = \Vert \vec{b} \Vert = 1 \right)$, Die tatsächlichen Ortsvektoren auf der Kugel sind damit

$$\vec{r}_{A} = R \vec{a}, \quad \land \quad \vec{r}_{B} = R \vec{b}$$

```{hint}

In einer realistischen Umsetzung würden wir $\vec{a}, \, \vec{b}$ aus Breite/Länge berechnen. Hier verwenden wir $\vec{a}, \, \vec{b}$ direkt, damit die Variationsidee im Vordergrund steht.
```

a) **Geometrie und Parametrisierung**

1. Begründe kurz, warum eine Kurve auf der Kugel als Funktion 

$$\vec{r}: \left[ 0, \, 1 \right] \rightarrow \mathbb{R}^{3}, \quad t \mapsto \vec{r}(t)$$

modelliert werden kann, wobei die Nebenbedingung $\Vert \vec{r}(t) \Vert$ = R für alle $t$ gilt und die Endpunkte fest sind:

$$\vec{r}(0) = R \vec{a}, \quad \land \quad \vec{r}(1) = R \vec{b}$$
...
2. Zeige, dass die Bogenlänge einer Kurve $\vec{r}(t)$ durch

$$S{\vec{r}} = \int_{t=0}^{1} \Vert \vec{r}^{\prime}(t) \Vert \, \mathrm{d}t$$
...
***Hinweis:*** *$\Vert \vec{r}^{\prime}(t) \Vert$ ist die Geschwindigkeit entlang der Kurve*

b) **Das Variationsproblem (mit Nebenbedingung)**

Wir möchten $S[\vec{r}]$ minimieren, **unter** der Nebenbedingung $\Vert \vec{r}^{\prime}(t) \Vert = R$. Dazu verwenden wir einen Lagrange-Multiplikator $\lambda (t)$ und betrachten das erweiterte Funktional

$$\tilde{S}[\vec{r}, \, \lambda] = \int_{t=0}^{1} \left\{ \frac{1}{2} \Vert \vec{r}^{\prime}(t) \Vert^{2} + \lambda(t) \left[ \Vert \vec{r}(t) \Vert^{2} - R^{2} \right] \right\} \, \mathrm{d}t$$

```{note}

Wir minimieren hier $\frac{1}{2} \Vert \vec{r}^{\prime} \Vert^{2}$ statt $\Vert \vec{r}^{\prime} \Vert$. Das ist eine Standardvereinfachung.

Wenn wir $t$ als "gleichmäßig entlang der Kurve" wählen, führt beides zur gleichen Geometrie der Minimalkurve (Geodäte).
```

1. Begründe kurz, warum diese Vereinfachung plausibel ist (Stichwort: Parametrisierung / "konstante Geschwindigkeit")
...
2. Berechne die Euler-Lagrange-Gleichung für $\vec{r}(t)$. Zeige, dass sie die Form

$$\boxed{\vec{r}^{\prime \prime}(t) = 2 \lambda(t) \vec{r}(t)}$$
...
3. Bestimme die Euler-Lagrange-Gleichung für $\lambda(t)$ und zeige, dass sie genau die Nebenbedingung erzwingt:

$$\boxed{\Vert \vec{r}(t) \Vert = R^{2}}$$
...
c) **Die Bahn liegt in einer Ebene**

1. Skaliere die Gleichung $\vec{r}^{\prime \prime}(t) = 2 \lambda(t) \vec{r}(t)$ mit einem Konstanten Vektor $\vec{n}$ (der unabhängig von $t$ ist) und argumentiere:

Wenn $\vec{n} \cdot \vec{r}(0) = 0$ und $\vec{n} \cdot \vec{r}^{\prime}(0) = 0$, dann gilt für alle $t$:

$$\boxed{\vec{n} \cdot \vec{r}(t) = 0}$$
...
***Hinweis:*** *Betrachte die Funktion $f(t) = \vec{n} \cdot \vec{r}(t)$ und zeige, dass sie eine lineare DGL zweiter Ordnung erfüllt.*

2. Wähle nun

$$\vec{n} \coloneqq \vec{a} \times \vec{b}.$$

Begründe:

- $\vec{n} \neq \vec{0}$, solange $\vec{a}$ und $\vec{b}$ nicht parallel sind,
- $\vec{n} \cdot \vec{a} = 0$ und $\vec{n} \cdot \vec{b} = 0$

Schlussfolgere: Die Minimalkurve liegt in der Ebene durch den Ursprung, die von $\vec{a}$ und $\vec{b}$ aufgespannt wird.
...
3. Warum ist die Schnittkurve "Ebene $\cap$ Kurve" ein Großkreis? Gormuliere das Ergenis in einem Satz.
...
Definiere den Winkel

$$\Omega = \arccos \left( \vec{a} \cdot \vec{b} \right).$$

1. Zeige, dass die Kurve

$$\boxed{\vec{r}(t) = R \left( \frac{\sin \left( \left( 1 - t \right) \Omega \right)}{\sin \left( \Omega \right)} \vec{a} + \frac{\sin \left( t \Omega \right)}{\sin \left( \Omega \right)} \vec{b} \right), \quad t \in \left[ 0, \, 1 \right]}$$

auf der Kugel liegt $\left( \Vert \vec{r}(t) \Vert = R \right)$ und die Endpunkte erfüllt.
...
2. Interpretiere diese Formel geometrisch: Warum "dreht" sie $\vec{a}$ in der Ebene auf $\vec{b}$ zu?
...
1. Markiere in dieser Aufgabe die Stelle, an der **Variationsrechnung** verwendet wurde
...
2. Markiere die Stelle, an der **Geometrie** das eigentliche Ergebnis geliefert hat.
...
3. Was sagt die Aufgabe über "cleveres Setup" und das Zusammenspiel von Mathematik und Physik aus?
